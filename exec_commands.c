...
int	exec_builtin(t_cmd_deque *cmd_line)
{
	...
	else if (ft_strncmp("pwd", cmd_line->head->commands_args[0], 4) == 0)
	{
		write(1, "이 자리에 pwd 와야 함\n", 29);
		//pwd 실행
	}
	else if (ft_strncmp("export", cmd_line->head->commands_args[0], 7) == 0)
	{
		write(1, "이 자리에 export 와야 함\n", 32);
		//export 실행
	}
	else if (ft_strncmp("unset", cmd_line->head->commands_args[0], 6) == 0)
	{
		write(1, "이 자리에 unset 와야 함\n", 31);
		//unset 실행
	}
	else if (ft_strncmp("env", cmd_line->head->commands_args[0], 4) == 0)
	{
		write(1, "이 자리에 env 와야 함\n", 29);
		//env 실행
	}
	else if (ft_strncmp("exit", cmd_line->head->commands_args[0], 5) == 0)
	{
